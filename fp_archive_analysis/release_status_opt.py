...
def load_src_fp(region, stereo_type, max_date_diff=None):
    ...
    regions = {'rema': {'project':'REMA', 'project': 'REMA', 'where':'y1 < -45'},
               'arcticdem': {'project':'ArcticDEM', 'project': 'ArcticDEM', 'where':'y1 > 48'},
               }
    ...
    ## Add season info
    if region == 'rema':
        src['season'] = src.apply(lambda x: determine_antarctic_season(x['acqdate']), axis=1)
    elif region == 'arcticdem':
        src['season'] = src.apply(lambda x: determine_arctic_season(x['acqdate']), axis=1)
...
#it = query_footprint('dg_imagery_index_stereo_cc20', where="y1 > 48", columns=['pairname', 'acqdate', 'sqkm_utm'])
#it['type'] = 'intrack'
...
#it_cols = list(it)
#it = gpd.sjoin(it, arc, how='inner', op='within')[it_cols]
#
### Determine if each footprint in arctic
##it['in'] = it.progress_apply(lambda x: find_within(arc.geometry[0], x), axis=1)
##it = it[it['in']==True]
#
### Determine season
#date_col = 'acqdate'
...
#    
## Use REMA/ArcticDEM to determine released not released
#pole = 'arctic'
#if pole == 'arctic':
...
############
#print('Xtrack')
### Do the same for xtrack
#xt = query_footprint('dg_imagery_index_xtrack_cc20', 
#                     where="project = 'ArcticDEM'", #REMA 
...
#xt_cols = list(xt)
#xt = gpd.sjoin(xt, arc, how='inner', op='within')[xt_cols]
##xt['in'] = xt.progress_apply(lambda x: find_within(arc.geometry[0], x), axis=1)
##xt = xt[xt['in']==True]
#
...
agg_df = plot_seasonal_agg_it_xt(results[0], 'area_sqkm', 'sum', labels=['Released Intrack', 'Unreleased Intrack'], barcolor='red', ylabel='Area km\u00b2')
agg_df = plot_seasonal_agg_it_xt(results[1], 'area_sqkm', 'sum', labels=['Released Cross-track', 'Unreleased Cross-track'],barcolor='blue', ylabel='Area km\u00b2')
