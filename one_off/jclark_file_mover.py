...
logger = create_logger('jclark_file_mover', 'sh',
                       'DEBUG')
            ...
            if f.endswith(('tif', 'ntf')):
                ...
                for d in dst:
                    ...
                    if not os.path.exists(dst_fp):
                        logger.info('Moving {} to: {}'.format(f, d))
                        # Move(?copy?) file to subdirectory
                        if not dryrun:
                            if move:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--move', action='store_true',
                        help='Flag to specify file moves, not copies.')
    ...
    file_mover(look_up, src_dir=src, dst_dir=dst, lut_sd=look_up_subdir, lut_id=look_up_id,
               move=move, dryrun=dryrun)
