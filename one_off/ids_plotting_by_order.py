...
missing_src = lookup_id_order(missing_ids, all_orders=None)
...
orders_val = orders.sort_values(by='ids')
...
with plt.style.context('seaborn-darkgrid', after_reset=True):
    ...
    ax2 = orders_val[orders_val.ids > n].plot.barh(y='ids', color='black', ax=ax2)
    ax2.set(xlabel='Number of missing IDs', ylabel='Order')
