...
def ahap_upload(server_loc, prj_dir, drives=None, dryrun=False, verbose=False):
    """
    ...
    if verbose:
        ...
        log_level = 'INFO'
    # create logger
    logger = create_logger('ahap_upload', 'sh', handler_level=log_level)
    logger = create_logger('ahap_upload', 'fh', handler_level=log_level, filename=log)
    ...
    # First level subdirectories on drives are './hsm{num}/' or './AHAP Tif files'
    ...
    logger.debug('AHAP Drives: {}'.format('\n'.join(ahap_drives)))
